...
#include "core.h"
	...
	/// @param logger [in] Pointer to a `CLogger` data structure. Pass `NULL` if not used
	...
	CLoggerInt32 clog_messagef(CLoggerLevel level, CLogger* logger, const char* location, const char* format,
							   va_list args);
	...
	/// @brief Log a message representing a `CLOGGER_LEVEL_INFO` level message
	/// @param location [in] Location of the log
	/// @param message [in] Format-able string message as you would use `printf()`
	/// @param ... [in] Variable-length args
	void clog_info(const char* location, const char* message, ...);

	/// @brief Log a message representing a `CLOGGER_LEVEL_DEBUG` level message
	/// @param location [in] Location of the log
	/// @param message [in] Format-able string message as you would use `printf()`
	/// @param ... [in] Variable-length args
	void clog_debug(const char* location, const char* message, ...);

	/// @brief Log a message representing a `CLOGGER_LEVEL_WARNING` level message
	/// @param location [in] Location of the log,
	/// @param message [in] Format-able string message as you would use `printf()`
	/// @param ... [in] Variable-length args
	void clog_warning(const char* location, const char* message, ...);

	/// @brief Log a message representing a `CLOGGER_LEVEL_ERROR` level message
	/// @param location [in] Location of the log
	/// @param message [in] Format-able string message as you would use `printf()`
	/// @param ... [in] Variable-length args
	void clog_error(const char* location, const char* message, ...);

	/// @brief Log a message representing a `CLOGGER_LEVEL_CRITICAL` level message
	/// @param location [in] Location of the log
	/// @param message [in] Format-able string message as you would use `printf()`
	/// @param ... [in] Variable-length args
	...
	/// @details This should be used in sync with the error/critical functions to make debugging/tracing easier as they
	/// will not trace back themselves.
	...
	/// @remarks In some IDEs, the traceback message printed to the console/terminal is clickable, allowing the end-user
	/// to jump to the point of interest.
